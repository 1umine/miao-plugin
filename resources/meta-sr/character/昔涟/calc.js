...
export const details = [
  {
    ...
    dmg: ({ talent, calc, attr }, { basic }) =>
      basic(calc(attr.hp) * talent.a["技能伤害"], "a"),
  },
  {
    title: "强化普攻·向着爱与明天♪·单体段",
    dmg: ({ talent, calc, attr }, { basic }) =>
      basic(calc(attr.hp) * talent.a2["单体伤害"], "a2"),
  },
  {
    title: "强化普攻·向着爱与明天♪·群体段(单目标)",
    dmg: ({ talent, calc, attr }, { basic }) =>
      basic(calc(attr.hp) * talent.a2["全体伤害"], "a2"),
  },
  {
    title: "忆灵技·花与箭的舞曲(单目标)",
    dmg: ({ talent, calc, attr }, { basic }) =>
      basic(calc(attr.hp) * talent.me["技能伤害"], "me"),
  },
  {
    title: "结界真实伤害·普攻单段",
  ...
  {
    title: ({ params } = {}) =>
      `结界真实伤害·强化普攻整套(按${params?.enemyCount || 1}目标)`,
    dmg: ({ talent, calc, attr, cons, params }, { basic }) => {
  ...
  {
    title: ({ params } = {}) =>
      `结界真实伤害·忆灵技整段(按${params?.enemyCount || 1}目标)`,
    dmg: ({ talent, calc, attr, cons, params }, { basic }) => {
      const enemyCount = Math.max(params.enemyCount || 1, 1)
      const trueDmgPct = getBarrierTrueDmgPct({ talent, cons, params })
  ...
  {
    title: ({ params } = {}) =>
      `献予「真我」之诗弹射总伤害(基础${params?.storyBounce || 0}次)`,
    dmg: ({ talent, calc, attr, cons, params }, { basic }) => {
      ...
      const bounceRet = basic(
        calc(attr.hp) * talent.me2["「真我」之诗•随机单体伤害"],
        "me",
      )
      return {
